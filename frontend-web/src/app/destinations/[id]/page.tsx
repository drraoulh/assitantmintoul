...
export default function DestinationDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 md:px-6">
      <div className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-[var(--green-deep)] via-[var(--green)] to-[var(--yellow)]/40">
        {site.images[0] ? (
          ...
          <div className="flex h-72 items-end p-8 md:h-96">
            <h1 className="font-display text-4xl text-white md:text-5xl">{site.name}</h1>
          </div>
        )}
      </div>
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1.4fr_1fr]">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {site.price ? <Badge tone="yellow">{site.price}</Badge> : null}
          </div>
          ...
          {site.culture ? (
            <section className="mt-8">
              <h2 className="font-display text-2xl text-[var(--green-deep)]">Culture</h2>
              <p className="mt-2 text-[var(--ink)]">{site.culture}</p>
            </section>
          ) : null}
          {site.activities?.length ? (
            <section className="mt-8">
              <h2 className="font-display text-2xl text-[var(--green-deep)]">Activités</h2>
          ...
          {site.sources?.length ? (
            <section className="mt-8">
              <h2 className="font-display text-2xl text-[var(--green-deep)]">
                {t('sources.title')}
              </h2>
              <ul className="mt-2 space-y-1 text-sm">
                {site.sources.map((s, i) => (
                  <li key={`${s.title}-${i}`}>
                    {s.url ? (
                      <a href={s.url} className="text-[var(--green)] underline" target="_blank" rel="noreferrer">
                        {s.title}
                      </a>
                    ) : (
                      s.title
